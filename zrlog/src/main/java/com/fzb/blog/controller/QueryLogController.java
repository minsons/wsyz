...
public class QueryLogController extends BaseController {
    ...
    public void all() {
        ...
        List<Type> types=new Type().getFirstRecords();
     /*   List<CollectLink> links=getAdaptTab(types.get(0).getInt("typeId"));
        data.put("firstTab", links);*/
        data.put("tabs",types);
    ...
    //获取指定的tab
    public List<CollectLink> getAdaptTab(int typeId){
    	return new CollectLink().queryLinkByTypeId(typeId);
    }
    
    //获取指定的tab
    public void  getAdaptTab(){
    	Integer typeId = getParaToInt("typeId");
    	List<CollectLink> dd=new CollectLink().queryLinkByTypeId(typeId);
    	renderJson("adaptTab",dd);
    }
